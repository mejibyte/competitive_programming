...
int main(){
  ...
  while (cin >> n && n){
    ...
    for (int i=0; i<n; ++i){
      for (int j=i; j<n; ++j){
	g[i][j] = g[j][i] = hypot(p[i].first - p[j].first, p[i].second - p[j].second) + 16.0;
	if (g[i][j] > longest){
	  start = i;
	  longest = g[i][j]; //Esta suposición da Wrong answer.
	}
      }
    }
    ...
    d[1<<start][start] = 0;
    q.push(state(start, 1<<start, 0.0, vector<int>(1, start))); //Esta suposición da Wrong answer.
