...
struct Comparator {
    vector<int> x, y, z;
    long long a, b, c, d, vpa, vpb, vpc, vpd;
    int first;
    Comparator(const vector<int> &x, const vector<int> &y, const vector<int> &z, long long a, long long b, long long c, long long d, int first) :
        x(x), y(y), z(z), a(a), b(b), c(c), d(d), first(first) { }
...
//                        System.err.println(Arrays.toString(facet.toArray()));
                    Comparator comp = Comparator(x, y, z, a, b, c, d, facet[0]);
                    sort(facet.begin() + 1, facet.end(), comp);
