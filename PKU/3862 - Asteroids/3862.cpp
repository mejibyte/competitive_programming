...
struct AngleComparator {
    Point pivot;
    AngleComparator(Point pivot) : pivot(pivot) {}
...
struct Polyhedron {
    ...
    double tetrahedronVolume(Point p1, Point p2, Point p3, Point p4) {
        Point u = p2 - p1;
        Point v = p3 - p1;
        Point w = p4 - p1;
    ...
    Point tetrahedronCentroid(Point p1, Point p2, Point p3, Point p4) {
        return Point((p1.x + p2.x + p3.x + p4.x) / 4.0, 
                     (p1.y + p2.y + p3.y + p4.y) / 4.0, 
                     (p1.z + p2.z + p3.z + p4.z) / 4.0);
    }
    
    Point centerOfMass(vector<Face> faces) {
        Point p1 = v[0];
...
int main(){
    ...
    cout << setprecision(10) <<  a.minDistance() + b.minDistance() << endl;
    return 0;
}