...
bool coneSegmentIntersection(Cone C, Point<long double> A, Point<long double> B) {
    ...
    // Now we have a cone whose apex is at the origin and opens up in the +z and -z directions
    //. We know its radius when z = -height (that's the radius we are given).
    // An equation for this cone is:
    // x^2 + y^2 = (radius / height)^2
    // This equation actually describes two cones:
    // i.e when looked from the side it looks like this:
    //
    //                    +z
    //                  \    /
    //                   \  /
    // -x ______________  \/  _______________ +x
    //                    /\
    //                   /  \
    //                  /    \
    //                    -z
    // so we have to be careful and check that the intersection happened in the lower part.
    
    // After replacing the parametric equation of the segment in the cone equation above, we get a
    // quadratic equation on t, the parameter for the segment equation.
    // Below are the coefficients for this equation: aa, bb and cc.

    long double u = C.r / height;
    long double dx = B.x - A.x, dy = B.y - A.y, dz = B.z - A.z;
    
    long double aa = dx*dx + dy*dy - u*u*dz*dz;
    long double bb = 2*(dx*A.x + dy*A.y - u*u*dz*A.z);
    long double cc = A.x*A.x + A.y*A.y - u*u*A.z*A.z;
