...
int node_stamp[MAXN], currentStamp, start;
...
int dfs(int u, int dir, int incoming) {
    ...
    int ans = -1;
    if (node_stamp[u] == currentStamp) ans = u;
    if (u == start and incoming != -1) return u; // full cycle
    ...
    for (int k = 0; k < 4; ++k) {
        ...
        if (cycle != -1 and cycle != u) ans = cycle;
        break;
