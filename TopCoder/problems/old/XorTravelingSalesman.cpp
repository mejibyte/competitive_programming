...
int XorTravelingSalesman::maxProfit(vector <int> cityValues, vector <string> roads) {
  ...
  been[0][cityValues[0]] = true;
  q.push( make_pair(0, cityValues[0]) );
  int best = cityValues[0];
  ...
  while (q.size() > 0) {
    ...
    for (int next = 0; next < n; ++next) {
      if (roads[at][next] == 'Y') {
        int new_profit = profit ^ cityValues[next];
        if (!been[next][new_profit]) {
          q.push( make_pair(next, new_profit) );
          been[next][new_profit] = true;
        }
      }
    }
  }
