...
long long FoxAndSorting::get(long long idx) {
    ...
    for (int l = 0; l < MAXL; ++l) ways[l][0] = l + 1;
    
    // ways[l][s] = How many numbers there are of length <= l such that the sum
    //              of their digits is exactly s. Leading zeros are allowed.
    for (int l = 1; l < MAXL; ++l) {
        for (int s = 1; s < MAXS; ++s) {
            for (int i = 0; i <= 9 and i <= s; ++i) {
                ways[l][s] += ways[l - 1][s - i];
            }
        }
    }
    ...
    for (requiredSum = 1; requiredSum < MAXS; ++requiredSum) {
        long long k = 0;
        for (int i = 1; i <= 9 and i <= requiredSum; ++i)
            k += ways[17][requiredSum - i];
        if (k >= idx) break;
        else idx -= k;
    }
    ...
    for (int len = 18; !(requiredSum == 0 and idx == 1); len--) {
        ...
        for (int i = (len == 18); i <= 9; ++i) {
            long long k = ways[len - 1][requiredSum - i];
