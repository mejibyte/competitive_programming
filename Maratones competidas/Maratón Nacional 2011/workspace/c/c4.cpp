...
void precompute(){
    ...
    for (int b = B - 1; b >= 0; --b) {
        for (int mask = 0; mask < (1 << 16); ++mask) {
            ...
            if ((bases[b] | mask) != mask) {
                if (mask < 1024) {
                    if (mask == 607) {
                        printf("Descartando mask "); binary(mask); printf(" porque no es compatible con bases[b=%d]", b); binary(bases[b]); puts("");
                       printf("bases[b] | mask = "); binary(bases[b] | mask);  puts(""); 
                    }  
                }
                continue;
            }
    ...
    for (int b = B - 1; b >= 0; --b) {
        for (int mask = 0; mask < (1 << 16); ++mask) {
            if (dp[b][mask] < oo) {
                printf("dp[%d][", b); binary(mask); printf("] = %d\n", dp[b][mask]);
            }
        }
    }
}
