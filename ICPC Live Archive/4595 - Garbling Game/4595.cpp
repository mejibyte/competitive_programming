...
inline void nextStep() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        newAdd[i] = add[i] + add[next[i]];
        if (newAdd[i] >= mod) newAdd[i] -= mod;
    }
    memcpy(next, newNext, n * sizeof(int));
...
int main(){
    int caseNumber = 0;
    while (scanf("%d %d %s", &R, &C, buf) == 3) {
        if (caseNumber++ > 0) puts("");
