...
Point reflect(Point p, const Point &a, const Point &b) {
    ...
    double u = ((p.x - a.x) * (b.x - a.x) + (p.y - a.y) * (b.y - a.y)) / distsqr(a, b);
    Point v; // Intersection of perpendicular segment
    v.x = a.x + u*(b.x - a.x);
    v.y = a.y + u*(b.y - a.y);
...
// Reflects the ray that goes in direction dir on the infinite line that 
// passes through a and b.
Point reflect_ray(const Point &dir, const Point &a, const Point &b) {
    Point normal = (b - a).normal();
    Point new_dir = dir;
    long double dot = 2 * (dir ^ normal);
    new_dir.x -= dot * normal.x;
    new_dir.y -= dot * normal.y;
    return new_dir;
    
    // Point normal(b.y - a.x, a.x - b.x);
    // long double length = sqrt(normal.x * normal.x + normal.y * normal.y);
    // normal.x /= length, normal.y /= length;
    // long double dot = 2 * (dir.x * normal.x + dir.y * normal.y);
    // Point new_dir(dir.x - dot * normal.x, dir.y - dot * normal.y);
    // return new_dir;
}
