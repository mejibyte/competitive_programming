...
// Like file.7.cpp, but use existing arrays instead of a new stack.
...
int main(){
    while (scanf("%d ", &F) == 1) {
        ...
        for (int i = F; i < N; ++i) {
            int from = i;
            while (head >= 0 and height[head] > lcp[i]) {
                add(start[head], i);
                from = start[head];
                head--;
            }
            if (head < 0 or lcp[i] > height[head]) {
                ++head;
                start[head] = from;
                height[head] = lcp[i];
            }
        }
        //assert(head >= 0 and height[head] == 0);
