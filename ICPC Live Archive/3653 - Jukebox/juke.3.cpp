...
int solve() {
    ...
    for (int remain = 0; remain < (1 << total_artists); ++remain) {
        int option = 0, covered_songs = 0;
        vector<int> longest_for_artist(total_artists, -1);
        vector<int> index_of_longest_for_artist(total_artists, -1);
        vector<int> golden_string(total_songs);
        ...
        for (int s = 0; s < total_songs; ++s) {
            ...
            if (len > longest_for_artist[my_artist]) {
                ...
                index_of_longest_for_artist[my_artist] = s;
            }
        }
            ...
            for (int len = 1; len <= artists[a].size(); ++len) {
                for (int i = 0; i + len - 1 < artists[a].size(); ++i) {
                    ...
                    if (songs_with_hash.count(hash) == 0) {
                        ...
                        option = option - longest_for_artist[a] + len;
                        covered_songs |= (1 << index_of_longest_for_artist[a]);
                        len = oo; break; // double break
                    } else {
                        ...
                        option = option - golden_string[song] + len;
                        covered_songs |= (1 << song);
        ...
        if (covered_songs == (1 << total_songs) - 1) {
            ans = min(option, ans);
        }
    }
    return ans;
