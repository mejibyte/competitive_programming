...
int next_z[MAXN][MAXN][MAXN];

int get_next_z(int x, int y, int z) {
   if (!covered[x][y][z]) return z;
   return next_z[x][y][z] = get_next_z(x, y, next_z[x][y][z]);
   
}

int compress(const vector<long long> &all, int what) {
